...
public class ListarEmpleadosDestacados extends JDialog {
	...
	public ListarEmpleadosDestacados() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(ListarEmpleadosDestacados.class.getResource("/visual/1046406.png")));
		this.code = "";
		setResizable(false);
	...
	public static void loadTableEmpleados(int selection){
		...
		case 0:
			for (Empleado aux : Empresa.getInstance().getMisEmpleados()) {
				if(aux.getEvaluacion()=="Destacado") {
				fila[0] = aux.getId();
         ...
         case 1:
        	 for (Empleado aux : Empresa.getInstance().getMisEmpleados()) {
 			  if(aux instanceof Jefe & aux.getEvaluacion()=="Destacado"){
 				fila[0] = aux.getId();
				fila[1] = aux.getNombre();
				fila[2] = aux.getDisponibilidad();
				if(aux.getProyectosTardios() == -1){
         ...
         case 2:
        	 for (Empleado aux : Empresa.getInstance().getMisEmpleados()) {
      			if(aux instanceof Programador & aux.getEvaluacion()=="Destacado"){
      				fila[0] = aux.getId();
     				fila[1] = aux.getNombre();
     				fila[2] = aux.getDisponibilidad();
     				if(aux.getProyectosTardios() == -1){
         ...
         case 3:
        	 for (Empleado aux : Empresa.getInstance().getMisEmpleados()) {
    			  if(aux instanceof Disenador & aux.getEvaluacion()=="Destacado"){
    				fila[0] = aux.getId();
   				fila[1] = aux.getNombre();
   				fila[2] = aux.getDisponibilidad();
   				if(aux.getProyectosTardios() == -1){
         ...
         case 4:
        	 for (Empleado aux : Empresa.getInstance().getMisEmpleados()) {
    			  if(aux instanceof Planificador & aux.getEvaluacion()=="Destacado"){
    				fila[0] = aux.getId();
   				fila[1] = aux.getNombre();
   				fila[2] = aux.getDisponibilidad();
   				if(aux.getProyectosTardios() == -1){
